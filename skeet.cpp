...
/************************
 * SKEET CLEAR ENTITIES
 * Delete all allocated entities and clear the shared list.
 ************************/
void Skeet::clearEntities()
{
...
/************************
 * SKEET SEND MESSAGE
 * Send a visitor message to every entity.
 ************************/
void Skeet::sendMessage(Message& message)
{
   for (auto entity : entities)
...
/************************
 * SKEET BIRD COUNT
 * Count birds currently stored in the shared entity list.
 ************************/
int Skeet::birdCount() const
{
   int count = 0;
   for (auto entity : entities)
      if (dynamic_cast<Bird*>(entity) != nullptr)
         count++;
   return count;
}

/************************
 * SKEET REMOVE ZOMBIES
 * Remove dead entities through the RemoveZombieEntity visitor.
 ************************/
void Skeet::removeZombies()
{
   RemoveZombieEntity removeZombie(entities, score);

   for (auto it = entities.begin(); it != entities.end();)
   {
      removeZombie.reset();
      (*it)->accept(removeZombie);

      if (removeZombie.shouldRemove())
      {
         delete* it;
...
void Skeet::animate()
{
   ...
   if (time.isStatus())
   {
      // get rid of all entities without changing the score
      clearEntities();
      return;
   }
   ...
   // move the birds, bullets, effects, and point values
   MoveEntity moveEntity(entities, hitRatio);
   sendMessage(moveEntity);

   // hit detection
   for (auto entityBird : entities)
   {
      Bird* bird = dynamic_cast<Bird*>(entityBird);
      if (bird == nullptr)
         continue;

      for (auto entityBullet : entities)
      {
         Bullet* bullet = dynamic_cast<Bullet*>(entityBullet);
         if (bullet == nullptr)
            continue;
         ...
         {
            for (int i = 0; i < 25; i++)
               entities.push_back(new Fragment(bullet->getPosition(), bullet->getVelocity()));
            bird->kill();
            bullet->kill();
            hitRatio.adjust(1);
   ...
   // remove dead birds, bullets, effects, and point values
   removeZombies();
}
...
void Skeet::drawLevel()
{
   ...
   // output the birds, bullets, fragments, and point values
   DrawEntity drawEntity;
   sendMessage(drawEntity);
...
void Skeet::spawn()
{
   ...
   case 1:
      ...
      if (birdCount() == 0 && random(0, 15) == 1)
         entities.push_back(new Standard(size, 7.0));
   ...
   case 2:
      ...
      // spawns when there is nothing on the screen
      if (birdCount() == 0 && random(0, 15) == 1)
         entities.push_back(new Standard(size, 7.0, 12));

      // spawn every 4 seconds
      if (random(0, 4 * 30) == 1)
         entities.push_back(new Standard(size, 5.0, 12));
      // spawn every 3 seconds
      if (random(0, 3 * 30) == 1)
         entities.push_back(new Sinker(size));
      break;
   ...
   case 3:
      ...
      // spawns when there is nothing on the screen
      if (birdCount() == 0 && random(0, 15) == 1)
         entities.push_back(new Standard(size, 5.0, 15));
      ...
      // spawn every 4 seconds
      if (random(0, 4 * 30) == 1)
         entities.push_back(new Sinker(size, 4.0, 22));
      // spawn every 4 seconds
      if (random(0, 4 * 30) == 1)
         entities.push_back(new Floater(size));
      break;
   ...
   case 4:
      ...
      // spawns when there is nothing on the screen
      if (birdCount() == 0 && random(0, 15) == 1)
         entities.push_back(new Standard(size, 4.0, 18));
      ...
      // spawn every 4 seconds
      if (random(0, 4 * 30) == 1)
         entities.push_back(new Sinker(size, 3.5, 25));
      // spawn every 4 seconds
      if (random(0, 4 * 30) == 1)
         entities.push_back(new Floater(size, 4.0, 25));
      // spawn every 4 seconds
      if (random(0, 4 * 30) == 1)
         entities.push_back(new Crazy(size));
      break;
